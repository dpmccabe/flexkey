...
describe Flexkey::Generator do
  describe '#new' do
    describe 'validations' do
      it 'raises an exception when format is missing' do
        expect { Flexkey::Generator.new(char_pool: { 'n' => :numeric }) }.to raise_error(Flexkey::GeneratorError, 'format is required')
      end

      it 'raises an exception when format is blank' do
        expect { Flexkey::Generator.new(format: '', char_pool: { 'n' => :numeric }) }.to raise_error(Flexkey::GeneratorError, 'format is required')
      end

      it 'raises an exception when char_pool is missing' do
        expect { Flexkey::Generator.new(format: 'nnn') }.to raise_error(Flexkey::GeneratorError, 'char_pool is required')
      end

      it 'raises an exception when char_pool has non-string letters' do
        expect { Flexkey::Generator.new(format: 'nnn', char_pool: { 123 => :numeric }) }.to raise_error(Flexkey::GeneratorError, 'char_pool letters must each be strings of length 1')
      end

      it 'raises an exception when char_pool has letters longer than 1 character' do
        expect { Flexkey::Generator.new(format: 'nnn', char_pool: { 'nn' => :numeric }) }.to raise_error(Flexkey::GeneratorError, 'char_pool letters must each be strings of length 1')
      end

      it 'raises an exception when char_pool has blank letters' do
        expect { Flexkey::Generator.new(format: 'nnn', char_pool: { '' => :numeric }) }.to raise_error(Flexkey::GeneratorError, 'char_pool letters must each be strings of length 1')
      end

      it 'raises an exception when no char_pool letters found in format' do
        expect { Flexkey::Generator.new(format: 'nnn', char_pool: { 'a' => :numeric }) }.to raise_error(Flexkey::GeneratorError, 'No char_pool letters present in format')
      end
    end
  ...
  describe 'accessors' do
    let(:key_gen) { Flexkey::Generator.new(format: 'nnn-aaa', char_pool: { 'n' => :numeric, 'a' => :alpha_upper }) }
  ...
  describe '#generate' do
    context 'when a single key is requested' do
      context 'with alphabetical and numeric character types' do
        subject { Flexkey::Generator.new(format: 'aaaaa-nnnn', char_pool: { 'a' => :alpha_lower, 'n' => :numeric }).generate }
      ...
      context 'with all character types' do
        ...
        it { is_expected.to match(/^[A-Z]-[a-z]-[0-9]-[ABCDEFGHJKLMNPQRTUVWXYZ]-[abcdefghjkmnpqrtuvwxyz]-[2346789]-[!@#\$%\^&*;:\(\)_\+-=\[\]{}\\\|'",.<>\/?]-[!@#\$%\^&\*;\:]$/) }
      end
    end

    context 'when the format and char_pool change' do
      let(:key_gen) { Flexkey::Generator.new(format: 'aaa-nnn', char_pool: { 'n' => :numeric, 'a' => :alpha_upper }) }
    ...
    context 'when multiple keys are requested' do
      let(:new_keys) { Flexkey::Generator.new(format: 'aaaaa-nnnn/cc', char_pool: { 'a' => :alpha_lower, 'n' => :numeric, 'c' => 'LMN' }).generate(5) }
    ...
    it 'raises an exception when too many keys are requested' do
      expect { Flexkey::Generator.new(format: 'nnn', char_pool: { 'n' => :numeric, 'a' => :alpha_upper }).generate(1001) }.to raise_error(Flexkey::GeneratorError, "There are only 1000 possible keys")
    end
  end
end
